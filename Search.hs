...
instance Eq s => Eq (Node s a) where
    (Node nodeS1 lastAction1 parentNode1 crtDepth1 nodeH1 succs1) ==
        (Node nodeS2 lastAction2 parentNode2 crtDepth2 nodeH2 succs2) = nodeS1 == nodeS2
...
nodeParent node = if crtDepth node  == 0 then Nothing else Just (parentNode node)
...
createStateSpace s = Node s Nothing undefined 0 0 child
  where 
    child = map (\(state, action) -> 
        createChildren action state (Node s Nothing undefined 0 0 child) 1 0) (successors s)
    
createChildren :: (ProblemState s a) => s -> a -> Node s a -> Int -> Float -> Node s a
createChildren s a parent depth heu = node
  where
    node = Node s (Just a) parent depth heu child
    child = map (\(state, action) -> createChildren action state node (depth + 1) 0) (successors s)
